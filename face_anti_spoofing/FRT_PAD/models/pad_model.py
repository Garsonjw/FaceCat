...
import argparse
from FRT_PAD.models.networks import Face_Recognition, Face_Expression, Face_Attribute_D
from FRT_PAD.models.networks import Baseline, GAT
...
def PA_Detector():
    ...
    pretrained_dict = torch.load('./FRT_PAD/pretrained_model/resnet18-5c106cde.pth')
...
def Face_Related_Work(downstream):
    ...
    if downstream == 'Face_Recognition':
        ...
        net = Face_Expression()
        model_path = './FRT_PAD/pretrained_model/ijba_res18_naive.pth.tar'
        model_dict = net.state_dict()
        pretrained_dict = torch.load(model_path)
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
        model_dict.update(pretrained_dict)
        ...
        model_path = './FRT_PAD/pretrained_model/celeba-128x128-5attrs/200000-D.ckpt'
        model_dict = net.state_dict()
        pretrained_dict = torch.load(model_path)
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
        model_dict.update(pretrained_dict)
