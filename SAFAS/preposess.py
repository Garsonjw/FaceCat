...
def proposess_video(video_path, savepath, sample_ratio=5, max=1000):
    ...
    while True:
        ...
        if frame_id % sample_ratio == 0:
            ...
            imageio.imwrite(os.path.join(savepath, f'org_{frame_id:04d}.jpg'), frame)
            imageio.imwrite(os.path.join(savepath, f'crop_{frame_id:04d}.jpg'), Image.fromarray(cropped))
            info_dict = {
                'box': box,
                'detect_prob': prob,
                'points': points,
    ...
    for i, file in enumerate(files):
        ...
        for image_id in range(frames_total):
            ...
            if not os.path.exists(square_image_path):
                ...
                imageio.imwrite(square_image_path, square_image)


if __name__ == '__main__':
    oulu_info = run_oulu(rootpath="../fasdata/OCIM/oulu/")
    msu_info = run_msu(rootpath="../fasdata/OCIM/MSU-MFSD-Publish/")
    casia_info = run_casia(rootpath="../fasdata/OCIM/CASIA_faceAntisp/")
    replay_info = run_replay(rootpath="../fasdata/OCIM/Replay/")
